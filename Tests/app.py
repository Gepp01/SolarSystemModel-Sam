...
app = Dash(__name__)
app.layout = html.Div([
    html.H1("Interactive 3D Solar System", style={'textAlign':'center'}),
    html.Div([
        dcc.Graph(figure=build_fig(['Mercury','Venus','Earth','Mars','Ceres'], 3, "Inner (Mercury→Mars)"), style={'width':'48%','display':'inline-block'}),
        dcc.Graph(figure=build_fig(['Earth','Mars','Jupiter','Saturn','Uranus','Neptune'], 35, "Main (Earth→Neptune)"), style={'width':'48%','display':'inline-block'}),
    ], style={'padding':'0 10px'}),
    html.Div([
        dcc.Graph(figure=build_fig(['Jupiter','Saturn','Uranus','Neptune','Pluto','Haumea','Makemake','Eris'], 70, "Outer & Dwarfs"), style={'width':'48%','display':'inline-block'}),
        dcc.Graph(figure=build_fig(['Earth','Jupiter','Neptune','Pluto','Eris','Sedna'], 110, "Extended (to Sedna)"), style={'width':'48%','display':'inline-block'}),
    ], style={'padding':'0 10px'})
])
